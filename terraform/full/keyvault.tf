...
resource "azurerm_key_vault" "keyvault_all" {
  ...
  soft_delete_retention_days = 7 # all is actually 90
...
resource "azurerm_key_vault_secret" "secret_all" {
  name         = var.secret_all_name
  value        = var.secret_all_name
  key_vault_id = azurerm_key_vault.keyvault_all.id
}

resource "azurerm_key_vault_key" "key_all" {
  name         = var.key_all_name
  key_vault_id = azurerm_key_vault.keyvault_all.id
  key_type     = "RSA"
  key_size     = 2048
...
resource "azurerm_key_vault_certificate" "certificate_all" {
  name         = var.certificate_all_name
  key_vault_id = azurerm_key_vault.keyvault_all.id
