...
resource "azurerm_key_vault" "keyvault_benchmark" {
  ...
  soft_delete_retention_days = 7 # benchmark is actually 90
...
resource "azurerm_key_vault_secret" "secret_benchmark" {
  name         = var.secret_benchmark_name
  value        = var.secret_benchmark_name
  key_vault_id = azurerm_key_vault.keyvault_benchmark.id
}

resource "azurerm_key_vault_key" "key_benchmark" {
  name         = var.key_benchmark_name
  key_vault_id = azurerm_key_vault.keyvault_benchmark.id
  key_type     = "RSA"
  key_size     = 2048
...
resource "azurerm_key_vault_certificate" "certificate_benchmark" {
  name         = var.certificate_benchmark_name
  key_vault_id = azurerm_key_vault.keyvault_benchmark.id
